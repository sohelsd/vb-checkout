    ...
    <header className="sticky top-0 z-50 overflow-x-hidden border-b border-gray-200 bg-white shadow-sm">
      <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-4 py-3">
            ...
            item.children ? (
              <div key={item.label} className="relative">
                <button
                  ...
                  className="flex items-center gap-1 text-base font-medium text-gray-700 transition-colors hover:text-primary"
                  aria-haspopup="true"
        ...
        <button
          ...
          className="shrink-0 flex flex-col gap-1.5 lg:hidden"
                ...
                {item.children ? (
                  <>
                    <button
                      type="button"
                      onClick={() => setFeaturesOpen(!featuresOpen)}
                      className="flex w-full items-center justify-between py-2.5 text-base font-medium text-gray-700"
                    >
                      {item.label}
                      <svg
                        className={`h-4 w-4 transition-transform ${featuresOpen ? 'rotate-180' : ''}`}
                  ...
                  <a
                    href={item.href}
                    className="block py-2.5 text-base font-medium text-gray-700 hover:text-primary"
                  >
                    {item.label}
                  </a>
                )}
